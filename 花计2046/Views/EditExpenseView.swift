...
struct EditExpenseView: View {
    ...
    @State private var editCurrency = ""
    ...
    init(expense: Expense, onSave: @escaping () -> Void) {
        ...
        _editCurrency = State(initialValue: expense.currency.isEmpty ? CategoryManager.currencySymbol : expense.currency)
    }
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            // 货币
                            VStack(alignment: .leading, spacing: 6) {
                                Text("货币").font(.system(size: 17)).foregroundColor(AppTheme.textSecondary)
                                Button(action: { showCurrencyPicker = true }) {
                                    HStack {
                                        if let current = currencyOptions.first(where: { $0.symbol == editCurrency }) {
                            ...
                            .sheet(isPresented: $showCurrencyPicker) {
                                CurrencyEditPicker(selection: $editCurrency, options: currencyOptions)
                                    .presentationDetents([.height(260)])
                            }
    ...
    func saveChanges() {
        ...
        if let idx = supabaseService.allRecords.firstIndex(where: { $0.id == updatedExpense.id }) {
            var records = supabaseService.allRecords; records[idx] = updatedExpense; supabaseService.allRecords = records
        }
...
struct CurrencyEditPicker: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Picker("货币", selection: $tempSelection) {
